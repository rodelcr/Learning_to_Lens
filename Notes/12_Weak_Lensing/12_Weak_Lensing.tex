...
identity that a single source produces strongly distorted images.
...
$n \sim 20$--$30$ galaxies per arcmin$^2$ in a few hours on a 4-m
telescope --- is the fundamental
...
\citet{bartelmann_1995} showed that a nearly identical relation
...
It follows that the tangential shear averaged on concentric circles
gives the azimuthally averaged mass profile of any lens, symmetric or
not.
...
\eqref{eq:wl_DeltaSigma} is the basic relation of cluster and galaxy--galaxy
...
(eq.~\ref{eq:wl_expectation_eps}).  This has an important consequence.
...
convergence, scaling $\kappab$ by $\lambda$ scales the shear by $\lambda$,
while the added \textit{uniform sheet} $(1-\lambda)$ contributes no shear
(its Hessian is isotropic).  Hence $\gammaL \to \gammaL' = \lambda\gammaL$, and
therefore
\begin{equation}
    \boxed{
...
$n(>S)/n_0(>S) \approx 1 + 2(\alpha - 1)\kappab$ in the weak limit for
unlensed counts $n_0(>S) \propto S^{-\alpha}$ (here $\alpha$ is the count
slope, not a deflection angle)
...
We now come to the method that made weak lensing quantitative:
...
\end{equation}
The complex kernel $\mathcal{D}$ is the shear produced by a
...
\begin{figure}[ht]
    ...
    $\theta_s = 1$.  The NFW tangential shear decreases monotonically: it
    tends to the finite value $\kappa_s/2$ at the centre, is nearly flat
    inside $\theta_s$, and steepens outside it, whereas the SIS is a single
    power law.  The measurable
...
distribution and hence cosmology \citep{gunn_1967, blandford_1991}
...
scale roughly as $\sigma_8\,\Omegam^{0.5}$, much like cluster abundances,
so cosmic shear is a leading probe of the clustering amplitude of matter
\citep[][Part~3, Sect.~6.4]{schneider_kochanek_wambsganss_2006}.
...
statistic~\eqref{eq:wl_Map} is a clean summary, because its
filter is so narrow that it probes $P_\kappa$ at essentially a single
multipole $\ell \sim 5/\theta$
(\citealp{schneider_kochanek_wambsganss_2006}, Part~3, eq.~109 and
Sect.~6.3).
...
first detected by \citet{brainerd_1996}).
...
\citep{brainerd_1996} yielded $\sigma_* \approx 160$~km\,s$^{-1}$ for an
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item For an NFW halo, sketch $\gammaL_{\mathrm t}(\theta)$ using
          Module~\ref{ch:axisymmetric_models} (eq.~\ref{eq:nfw_gamma}).
          Show that it stays finite at the centre, and explain
          qualitatively why its logarithmic slope changes near $\theta_s$,
          unlike the single power law of the SIS.
\end{enumerate}
Verify with
\solnlink{12_Weak_Lensing/problems\_12.wl}{problems\_12.wl}.
\end{exercise}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item For a scale-free power spectrum $P_\kappa(\ell) = A\,\ell^{-1}$,
          use $\int_0^\infty J_n(\ell\theta)\,d\ell = 1/\theta$ (valid for
          $n = 0$ and $n = 4$) to compute $\xi_+(\theta)$ and
          $\xi_-(\theta)$.  Why would the steeper $P_\kappa \propto \ell^{-2}$
          not give a finite $\xi_+$?
