...
\begin{quote}
\textit{In the special case of a circularly symmetric lens, we can shift the
coordinate origin to the center of symmetry and reduce light deflection to a
one-dimensional problem.}
\hfill --- Narayan \& Bartelmann (1997), Sect.~2.1.2
\end{quote}
...
Take $\theta$ as a signed coordinate along the line through the lens
and the source ($\beta \geq 0$; $\theta > 0$ on the source's side).  The
SIS deflection has constant magnitude $\thetaE$ and points along
$\theta/|\theta|$, so the 1D lens equation is
\begin{equation}
    \beta = \theta - \thetaE\,\frac{\theta}{|\theta|} \,.
    \label{eq:sis_lens_eq}
\end{equation}
This is \textit{piecewise linear} in $\theta$ (unlike the point mass lens
...
the lens from the source.  (For $\theta > 0$,
eq.~\eqref{eq:sis_lens_eq} reads $\beta = \theta - \thetaE$, giving
$\theta_+$; for $\theta < 0$ it reads $\beta = \theta + \thetaE$,
giving $\theta_-$, which is indeed negative when $\beta < \thetaE$.)
...
\end{equation}
The $\theta < 0$ branch would give $\theta = \beta - \thetaE > 0$,
which contradicts $\theta < 0$, so there is no second image.
...
This is the projection of the 3D density
(Congdon \& Keeton eq.~2.53)
\begin{equation}
    \rho(r) = \frac{\sigma_v^2}{2\pi\Gnewton\,(r^2 + r_c^2)} \,,
    \label{eq:nis_rho}
\end{equation}
with physical core radius $r_c = \Dd\,\theta_c$: since
$\int_{-\infty}^{\infty} dz/(\xi^2 + r_c^2 + z^2) = \pi/\sqrt{\xi^2 + r_c^2}$,
integrating along the line of sight at $\xi = \Dd\theta$ gives
eq.~\eqref{eq:nis_sigma}.
...
\begin{itemize}
    ...
    \item The \textbf{concentration parameter}
          ...
          Typical values are $c \sim 5$ for massive clusters
          ($M_{200} \sim 10^{15}\,\Msun$) and $c \sim 10$ for
          galaxy-mass haloes ($M_{200} \sim 10^{12}\,\Msun$)
          \citep[][Part~2, p.~128]{schneider_kochanek_wambsganss_2006}.
\end{itemize}
