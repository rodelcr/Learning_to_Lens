...
\begin{itemize}[nosep]
    \item \mathlink{01c_Schwarzschild/schwarzschild_metric.wl}{schwarzschild\_metric.wl}
          ...
          and compute the Kretschmann scalar
...
precision: E\"otv\"os-type torsion-balance experiments (Dicke,
Braginsky, and the E\"ot-Wash group) verified this to better than one
part in $10^{13}$, and the MICROSCOPE satellite mission (2022) extended
the limit by two orders of magnitude:
\begin{equation}
    \frac{|m_i - m_g|}{m_g} < 10^{-15} \,.
...
\textbf{geodesic} (Module~1b, Section~\ref{sec:geodesic_equation}).
...
symmetries can be written (Carroll eq.~5.11; cf.\ Congdon \& Keeton
eq.~3.64):
\begin{equation}
...
\begin{figure}[ht]
    ...
    $z \approx 0.3$).  For the Sun, $z \approx 2.1 \times 10^{-6}$.}
...
only $r = 0$ is a true singularity.  We verify eq.~\eqref{eq:kretschmann}
...
\end{equation}
where $r_*$ (units of length) is the \textbf{tortoise coordinate}
(Carroll eq.~5.108), and dividing by $c$ makes $r_*/c$ a time shift
so that $v$ is a well-defined time coordinate in SI units.
In terms of $(v, r, \theta, \phi)$, the Schwarzschild line element
becomes (Carroll eq.~5.111):
\begin{equation}
    \boxed{
...
\begin{equation}
    ds^2 \approx -\left(1 - \frac{R_S}{\rho}\right) c^2\, dt^2
    + \left(1 + \frac{R_S}{\rho}\right)
    \bigl(d\rho^2 + \rho^2\, d\Omega^2\bigr) .
    \label{eq:isotropic_weak_field}
\end{equation}
Since $R_S = 2GM/c^2$, we have $R_S/\rho = 2GM/(\rho\, c^2) =
-2\Phi/c^2$ where $\Phi = -GM/\rho$ is the Newtonian potential.
...
\midrule
Earth   & $5.97 \times 10^{24}$ kg  & 8.9 mm     & 6371 km \\
