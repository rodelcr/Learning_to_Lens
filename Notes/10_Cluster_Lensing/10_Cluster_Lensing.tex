...
\begin{quote}
\textit{In the hierarchical model of structure formation, galaxy clusters are the
largest and the youngest structures in the universe.  They form through the
assembly of smaller-scale structures, and for this reason, they are often
observed during mergers.}
\hfill --- M.~Meneghetti, \textit{Introduction to Gravitational Lensing} (2021), Sect.~5.7
\end{quote}
...
\begin{enumerate}
    \item \textbf{Giant arcs.}  Extended background galaxies are
          ...
          The first giant arcs were discovered independently by
          Lynds \& Petrosian (1986) and Soucail et al.\ (1987),
          with the gravitational lensing interpretation confirmed
          spectroscopically by Soucail et al.\ (1988).
...
($\Dd \approx 919$~Mpc, $\Ds \approx 1727$~Mpc,
$\Dds \approx 1328$~Mpc, from Module~\ref{ch:cosmological_distances}):
\begin{itemize}[nosep]
    \item $r_{200} \approx 1.86$~Mpc, $r_s = r_{200}/c \approx 372$~kpc,
    \item $\theta_s = r_s / \Dd \approx 84''$,
    \item $\kappa_s \approx 0.51$.
\end{itemize}
The local convergence $\kappab = \kappa_s f(x)$ exceeds unity for
$x \lesssim 0.11$ (i.e., $\theta \lesssim 9''$), and the mean convergence
$\bar\kappa$ reaches unity at $x \approx 0.18$ ($\theta \approx 15''$), so the
cluster has a central region of super-critical convergence and is
therefore a strong lens.
...
\begin{equation}
    \frac{2\kappa_s}{x_t^2}\left[\ln\frac{x_t}{2} + g(x_t)\right] = 1 \,,
...
\begin{equation}
    2\kappa_s\, f(x_r)
    - \frac{2\kappa_s}{x_r^2}\left[\ln\frac{x_r}{2} + g(x_r)\right] = 1 \,.
...
the tangential critical curve lies at $x_t \approx 0.18$, giving
$\theta_t \approx 15''$ --- a physical radius of $\sim 66$~kpc at the
cluster redshift.

\begin{remark}
The tangential critical curve radius is often called the
\textbf{Einstein radius} of the cluster, $\thetaE$.  A \emph{spherical}
NFW halo with $M_{200} \sim 10^{15}\,\Msun$ and $c \sim 5$ typically has
$\thetaE \sim 10''$--$20''$ at these redshifts.  Observed clusters
(Abell~1689, MACSJ~0717, etc.) frequently display $\thetaE \sim
30''$--$50''$; this additional lensing strength comes from
ellipticity, substructure, merger boosts, and departures of the true
mass distribution from a smooth spherical NFW.  For a galaxy lens the
Einstein radius is instead $\sim 1''$, and the enclosed projected
mass within $\thetaE$ ranges from $\sim 10^{11}\,\Msun$ (galaxies) to
$\sim 10^{13}$--$10^{14}\,\Msun$ (clusters).
\end{remark}
...
\begin{equation}
    M(\thetaE) \approx 1.3 \times 10^{14}\,\Msun \,.
...
\begin{itemize}
    \item \textbf{Earendel} (WHL0137-LS): The most distant individual
          ...
          $\mu \sim 1000$--$40{,}000$ by the cluster WHL~0137-08
          \citep{welch_2022}.  The extreme magnification occurs
...
lensing.  Without magnification, Earendel would have an apparent
magnitude of $m \approx 35$--$39$ --- utterly undetectable.  A
magnification of $\mu \sim 1000$--$40{,}000$ corresponds to
$\Delta m \approx -7.5$ to $-11.5$ magnitudes, bringing it to the observed
$m \approx 27.2$, within reach of HST and JWST \citep{welch_2022}.
Such extreme magnification requires the image to lie within a few
hundredths of an arcsecond of the critical curve ($\lesssim 0.036''$ in
the \citet{welch_2022} lens models), making such discoveries exceedingly
rare.
\end{remark}
...
\begin{itemize}
    ...
    \item Clusters serve as \textbf{gravitational telescopes},
          ...
          like Earendel ($z = 6.2$, $\mu \sim 10^3$--$10^4$) and galaxies
          at $z > 10$.
...
$\Dd = 919$~Mpc, $\Ds = 1727$~Mpc, $\Dds = 1328$~Mpc.
...
with $\thetaE = 30''$.  (For reference, $\Dd = 919$~Mpc,
$\Ds = 921$~Mpc and $\Dds = 802$~Mpc; at $z_s = 9$ the angular-diameter
distance has turned over, so $\Ds$ is barely larger than $\Dd$.  Parts
(a)--(c) need only $\thetaE$.)
\begin{enumerate}[label=(\alph*)]
...
source at $z_s = 2$, using $\Dd = 919$~Mpc, $\Ds = 1727$~Mpc,
$\Dds = 1328$~Mpc:
\begin{enumerate}[label=(\alph*)]
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item Convert $\bar{\kappa}(\theta)$ to the projected mass
          ...
          Use $\Sigmacr \approx 2.8\ee{15}\,\Msun\,\mathrm{Mpc}^{-2}$
          for $z_d = 0.3$, $z_s = 1$ (a typical background redshift
          for weak lensing), and $\Dd = 919$~Mpc.
