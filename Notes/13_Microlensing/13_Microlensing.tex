...
\begin{quote}
\textit{``Of course, there is not much hope of observing this phenomenon
directly.''}
\hfill --- A.~Einstein (1936), on star--star lensing,\\
\hfill as quoted by J.~Wambsganss in Schneider, Kochanek \& Wambsganss (2006), Part~4
\end{quote}

\noindent
Einstein's pessimism \citep{einstein_1936} was well founded for the
technology of his day, but wrong for ours.  The images formed when one star
lenses another are split by only $\sim\!10^{-3}$~arcsec, far too small to
resolve, so the
...
$\thetaE \sim 10^{-3}$~arcsec (a milliarcsecond, far below the arcsecond
image splittings of galaxy lensing).  The two images produced by a point lens straddle the lens and are
separated by $\Delta\theta \approx 2\thetaE \sim$~mas, far below the
resolution of any conventional telescope.

As a result, \textbf{individual image positions,
...
\begin{equation}
    ...
    \label{eq:micro_tau}
\end{equation}
The optical depth is \emph{independent of the individual lens masses}: it
...
Paczy\'nski's motivation was this: if the dark halo of the Milky Way were
...
$\tau_{\mathrm{LMC}} \approx 1.2^{+0.4}_{-0.3} \times 10^{-7}$ (MACHO, with a
further $\sim\!20\%$ systematic uncertainty), is far too low for compact
objects to constitute the entire halo.  EROS limits objects of
$2\times10^{-7}$ to $1\,\Msun$ to less than $25\%$ of the halo, MACHO finds
that objects of $0.3$--$30\,\Msun$ cannot make up the whole halo, and the
surviving events are consistent with ordinary stars in the disk or within the
Clouds themselves \citep[Sect.~3.5]{schneider_kochanek_wambsganss_2006}.
This important \emph{null} result rules out compact objects of
$\sim\!10^{-7}$--$30\,\Msun$ as the dominant component of the halo dark
matter.
...
$\tau_{\mathrm{bulge}} \approx (0.9\text{--}4) \times 10^{-6}$ (EROS to
MACHO/OGLE), tracing the
...
axis.  The caustic topology changes qualitatively with $d$.  For equal
masses: widely separated lenses ($d > 2$) have two small four-cusp
(astroid-shaped) caustics; for $2^{-1/2} < d < 2$ these are merged into a
single six-cusp caustic; and for $d < 2^{-1/2} \approx 0.707$ the caustic
splits into a central four-cusp caustic plus two small triangular ones
\citep{schneider_weiss_1986, petters_singularity_2001}
\citep[Part~4, Sect.~2.1]{schneider_kochanek_wambsganss_2006}.%
\footnote{\citet{schneider_weiss_1986}, reproduced as Fig.~4 of
\citet[Part~4]{schneider_kochanek_wambsganss_2006}, quote these transitions
as $1$ and $8^{-1/2}$ because they measure the \emph{half}-separation of the
two lenses.  In the full-separation units used here the transitions are at
$d = 2$ and $d = 2^{-1/2}$; both values are checked by counting critical
curves in \texttt{microlensing.wl}.}

\subsection{Caustic Crossings}

The observational signature is a sharp spike.  When the source \emph{crosses} a
...
$t_{\mathrm{cross}} = R_{\mathrm{source}}/v_\perp$ it takes the source to move
by its own radius.  Caustic-crossing binaries thus break the point-source
...
\citep[Sect.~4.1]{schneider_kochanek_wambsganss_2006}.  By
...
required.  Microlensing is especially sensitive to \emph{cool, low-mass
planets at $\sim\!1$--few~AU} around distant ($\sim$~kpc) stars, a region of parameter
...
\citep[Sect.~7.1]{schneider_kochanek_wambsganss_2006}.  For the concordance
cosmology of Module~\ref{ch:cosmological_distances}
($\Dd \approx 1260$, $\Ds \approx 1730$, $\Dds \approx 1100$~Mpc),
\begin{align}
    r_{\mathrm{E}} &= \sqrt{\frac{4\Gnewton M}{\speedoflight^2}\,
    \frac{\Ds\,\Dds}{\Dd}}
    \approx 5 \times 10^{16}\,
    \left(\frac{M}{\Msun}\right)^{1/2}\,\text{cm} \,,
    \label{eq:micro_qso_rE} \\
    t_{\mathrm{E}} &= \frac{r_{\mathrm{E}}}{v_\perp} \approx 28\,
    \left(\frac{M}{\Msun}\right)^{1/2}
    \left(\frac{v_\perp}{600~\text{km s}^{-1}}\right)^{-1}\,\text{yr} \,,
    \label{eq:micro_qso_tE}
\end{align}
where $v_\perp$ is the relative transverse velocity projected into the
source plane.%
\footnote{\citet[Part~4, Sect.~7.1]{schneider_kochanek_wambsganss_2006} quotes
$r_{\mathrm{E}} \approx 4\times10^{16}$~cm, which corresponds to an older
Einstein--de~Sitter-type cosmology (EdS with $H_0 = 70$ gives
$3.8\times10^{16}$~cm).  It also quotes $t_{\mathrm{E}} \approx 15$~yr and
$t_{\mathrm{cross}} \approx 4$~months.  Those two values are consistent with
each other but are about $1.5\times$ shorter than $r/v$ for
$v = 600$~km\,s$^{-1}$, which implies a differently defined effective velocity.
Here we quote $r/v$ directly; see \texttt{problems\_13.wl}.}
...
time $t_{\mathrm{cross}} = R_{\mathrm{source}}/v_\perp \approx 6\,R_{15}\,
(v_\perp/600~\text{km s}^{-1})^{-1}$~months, where the quasar continuum radius
$R_{15}$ is in units of $10^{15}$~cm.  The scale estimates and the
$\propto\sqrt{M}$ mass scaling are checked in
\solnlink{13_Microlensing/problems\_13.wl}{problems\_13.wl}.

The value of quasar microlensing is that $t_{\mathrm{cross}}$ ---
...
images vary independently by up to $\sim\!1.3$~mag; the fits imply a continuum
...
\begin{itemize}
    ...
    \item The \textbf{optical depth}
          ...
          $\tau \approx 1.2\times10^{-7}$ rules out compact objects of
          $\sim\!10^{-7}$--$30\,\Msun$ as the dominant halo dark matter
          (EROS: $< 25\%$ of the halo for $2\times10^{-7}$--$1\,\Msun$);
          toward the bulge $\tau \sim 10^{-6}$.
