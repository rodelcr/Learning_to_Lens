...
gravitational lensing event occurs. [\ldots] The results of such an
investigation depend on the assumed distribution of lens masses and
their individual density profiles.  A comparison of these results with a
statistically well defined sample of observed lens cases can in principle
allow one to constrain the lens contents and/or the geometry of the
Universe.}
\hfill --- P.~Schneider, in Schneider, Kochanek \& Wambsganss (2006), Part 1, \S5
\end{quote}
...
\begin{itemize}[nosep]
    \item \mathlink{11_Lensing_Statistics/lensing_statistics.wl}{lensing\_statistics.wl}
          ...
          SIS cross-sections (including the combined SIS threshold
          cross-section, Eq.~\eqref{eq:sis_cross_full}), the comoving
          optical-depth integrand and its $\OmegaL$ dependence, and the
...
\begin{equation}
    ...
    = \pi\,\thetaE^2
      ...
      H(2\thetaE - \Delta\theta),
    \label{eq:sis_cross_full}
\end{equation}
where $H$ is the Heaviside step function and $r$ is finite, so that
$(r-1)/(r+1) < 1$ keeps the source inside the multiple-imaging disk.%
\footnote{The printed eq.~101 of \citet{schneider_kochanek_wambsganss_2006}
carries an extra factor $H(\mu - 2)$.  That factor would set
$\sigma = 0$ for any magnification threshold $\mu < 2$, yet every SIS
image pair has total magnification $2/y \ge 2$, so such a threshold is
always met.  Direct integration over the source plane confirms the form
above for all $\mu$ (companion script, Test~6).}
Because the SIS Einstein radius scales as
$\thetaE \propto \sigma_v^2$ (Eq.~\eqref{eq:sis_einstein_radius}), the
multiple-imaging cross-section scales as
$\sigma \propto \thetaE^2 \propto \sigma_v^4$, so lens statistics are
...
volume factor \citep{turner_ostriker_gott_1984}.  Because this volume
factor is larger when $\OmegaL > 0$ (for fixed $n_{\mathrm{com}}$ and
$z_s = 2$, about three times larger in a flat $\Omegam = 0.3$ universe
than in an Einstein--de~Sitter one), lens counts were an early probe of
the cosmological constant
\citep[][Part~2, \S6]{schneider_kochanek_wambsganss_2006}.  The same
calculation underpins forecasts of $\sim 10^5$ galaxy-scale lenses
discoverable in \textit{Euclid} and LSST imaging \citep{collett_2015}.
...
\citep[][Part~1, \S5.3]{schneider_kochanek_wambsganss_2006}.  Two cases
follow.  For \textbf{power-law counts}
$N_0(>S) = A\,S^{-\beta}$,
\begin{equation}
    N(>S) = N_0(>S)\,
...
Because $p(\mu) \propto \mu^{-3}$ at high magnification
\citep{blandford_narayan_1986} --- a consequence of the universal fold
magnification law, Eq.~\eqref{eq:fold_magnification}, met for the point
mass in \S\ref{sec:cross_point} --- the bias integral grows sharply as
